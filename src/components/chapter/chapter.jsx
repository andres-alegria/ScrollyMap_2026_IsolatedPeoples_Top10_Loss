...
import useLineSplit from './use-line-split';
...
const StageComponent = isStage ? STAGES[stage] : null;
  ...
  const stepClasses = cx(
    isStage
      ? "step w-full opacity-100"
      // card chapters sit over the globe: wider measure, and no inactive fade,
 ...
 const renderLegend = (legend, sources) => (
  ...
  useLineSplit(descriptionRef, splitLines);
  ...
  //
  // Not for card chapters: this is measured from a ref, so it is empty on the
  // first render and only becomes 'my-40' once a re-render has happened —
  // moving the card 160px down mid-session. Those chapters drive the globe's
  // timing, which cannot be tuned against a position that shifts. They carry
  // their own band padding and sit in a 120vh step, so they don't need it.
  const extraHeight = !card && chapterHeight && chapterHeight < 400 ? 'my-40' : '';

  return (
  <div id={id} className={cx(classList, isStage || card ? "w-full" : ALIGNMENTS[alignment])}>
      <Waypoint
        onEnter={onEnter}
        onLeave={onLeave}
        topOffset="-20%"
...
<StageComponent
  chapter={{ id, rank, title, homeTo, description, content, html, legend }}
  image={image}
  {...stageProps}
/>
  </div>
  ...
  <div
    ...
    className={cx(
      theme,
      card ? "chapter-card" : "rounded-lg p-6 space-y-4",
